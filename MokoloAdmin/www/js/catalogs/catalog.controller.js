(function() {
	...
	CatalogController.$inject = [
		'catalogsService', '$stateParams', 'cameraService', 'amazonS3Service', '$ionicLoading', '$window', '$state', 'fieldEditor', 'listsService', '$ionicPopup', '_', 'ENV'];
	...
	function CatalogController(
		...
		function changeString(field) {
			fieldEditor.showTextFieldEditor({
				...
				var obj = {};
				obj[field] = value;
				catalogsService.saveItem(vm.item.$id, obj);
			});
		}
		...
		function changeDescription() {
			$state.go('app.edit-Catalogdescription', {
				id: $stateParams.id
			});
		}
