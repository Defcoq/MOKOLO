(function() {
	...
	function fieldEditor($rootScope, $ionicModal, $q, _) {
		...
		function createMultiSelectorVm(defer, modal, options) {
			var vm = {
				...
				isItemSelected: function(item) {
					return _.some(vm.selectedItems, '$id', item.$id);
				},
				save: function(data) {
					modal.hide();
					defer.resolve(data);
		...
		function showSelector(options) {
			var templateUrl
			if (options.multi) {
				templateUrl = 'js/common/services/field-editor/multi-selector.html';
				return show(options, templateUrl, createMultiSelectorVm);
			}
			templateUrl = 'js/common/services/field-editor/selector.html';
			return show(options, templateUrl, createSelectorVm);
		}

		function showTextFieldEditor(options) {
			var templateUrl = 'js/common/services/field-editor/text-field-editor.html';
